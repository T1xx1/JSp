...
:::danger[Important]
JS+ will only be able to parse this syntax correctly if the negative subscript is explicitly stated since it only perform static analysis and not runtime checks.
